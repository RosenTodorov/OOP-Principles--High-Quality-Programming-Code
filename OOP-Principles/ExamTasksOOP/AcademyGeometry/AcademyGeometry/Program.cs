    ...
    public class AdvanceFigureController : FigureController
    {
        ...
        public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
        {
            ...
            public double GetArea()
            {
                ...
                double height = GetHeight(); //•	Vector – provides an abstract class for n-dimensional vectors, with a property for Magnitude (length) and a field for the components of the vector

                double basePerimeter = 2 * Math.PI * this.Radius;

                double sideArea = basePerimeter * height;

                return topAndBottomArea + sideArea; //cqlata area - plo6t
            }
